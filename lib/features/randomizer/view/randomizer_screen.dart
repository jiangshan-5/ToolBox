...
class _RandomizerScreenState extends ConsumerState<RandomizerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('高自由度决策随机沙盒', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: AnimatedSwitcher(
                      ...
                      child: _buildActiveSandbox(state, notifier),
                    ),
                  ),
                ),
              ],
  ...
  Widget _buildModeSelector(RandomizerState state, RandomizerNotifier notifier) {
    final modes = [
      {'title': '区间随机沙盒', 'mode': RandomizerMode.number, 'icon': Icons.tune_rounded, 'color': Colors.cyanAccent},
  ...
  Widget _buildActiveSandbox(RandomizerState state, RandomizerNotifier notifier) {
    switch (state.mode) {
      case RandomizerMode.number:
        return _buildNumberSandbox(state, notifier);
      case RandomizerMode.list:
        return _buildWeightedSandbox(state, notifier);
      case RandomizerMode.dice:
        return _buildDiceSandbox(state, notifier);
    }
  }
  ...
  Widget _buildNumberSandbox(RandomizerState state, RandomizerNotifier notifier) {
    return Column(
      ...
      children: [
                ...
                ...List.generate(state.customRanges.length, (index) {
                  ...
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        ScaleOnTap(
                          onTap: () => notifier.toggleRangeActive(index, !range.active),
                          child: Checkbox(
                            value: range.active,
                        ...
                        Expanded(
                          child: _buildSmallNumberInput(
                            ...
                            onChanged: (v) => notifier.updateRangeMax(index, int.tryParse(v) ?? 100),
                          ),
                        ),
                      ],
                    ),
        ...
        HoverGlowCard(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('定制生成数字排版后缀', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 16),
                Row(
                  children: [
                    Expanded(
        ...
        // RUN BUTTON & RESULTS
        ScaleOnTap(
          onTap: state.isGenerating ? null : notifier.runCombinatorialGenerate,
          child: Container(
            width: double.infinity,
            height: 58,
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [Color(0xFF00FFFF), Color(0xFF0080FF)]),
            ...
            child: Center(
              child: state.isGenerating
                  ...
                  : const Text(
                      '一 键 沙 盒 区 间 抽 选',
                      style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 15, letterSpacing: 1.1),
                    ),
            ),
          ),
        ),
    ...
    return Column(
      ...
      children: [
            ...
            ScaleOnTap(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(color: Colors.pinkAccent.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))
                  ],
                ),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                ScaleOnTap(
                                  onTap: () => notifier.removeWeightedOption(item.id),
                                  child: const Padding(
                                    padding: EdgeInsets.all(8.0),
                                    child: Icon(Icons.delete_outline_rounded, color: Colors.white30, size: 18),
                                  ),
                                ),
                              ],
                            ),
        ...
        if (state.weightedOptions.isNotEmpty) ...[
          ...
          // DRAW ACTION BUTTON
          ScaleOnTap(
            onTap: state.isGenerating ? null : notifier.runWeightedDecision,
            child: Container(
              width: double.infinity,
              height: 58,
              decoration: BoxDecoration(
        ...
        HoverGlowCard(
          ...
          child: Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('自由定制命运骰表面项 (逗号分割开启自由文本骰子)', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 13)),
                const SizedBox(height: 16),
                TextFormField(
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      // Satisfying Rotation & bounce physics
                      TweenAnimationBuilder<double>(
                        tween: Tween(begin: 0, end: _isDiceBouncing ? 6 * 3.14159 : 0),
