...
class _ConverterScreenState extends ConsumerState<ConverterScreen> {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        if (category == ConverterCategory.sandbox) ...[
                          Consumer(
                            builder: (context, ref, child) {
                              final activeCustomId = ref.watch(
                                converterProvider.select(
                                  (s) => s.activeCustomId,
                                ),
                              );

                              final customConverters = ref.watch(
                                converterProvider.select(
                                  (s) => s.customConverters,
                                ),
                              );

                              return _buildSandboxSelectorPanel(
                                activeCustomId,
                                customConverters,
                                notifier,
                              );
                        ...
                        Consumer(
                          builder: (context, ref, child) {
                            final allConversionsMatrix = ref.watch(
                              converterProvider.select(
                                (s) => s.allConversionsMatrix,
                              ),
                            );
  ...
  Widget _buildCategorySelector(
    ConverterCategory activeCat,
    ConverterNotifier notifier,
  ) {
    final categories = [
      {
        'title': '长度',
        'cat': ConverterCategory.length,
  ...
  Widget _buildSandboxSelectorPanel(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSmallFormInput(
                    controller: _customNameController,
                    label: '转换器标题 (如: 汇率、游戏点数换算)',
                  ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Consumer(
            builder: (context, ref, child) {
              final fromUnit = ref.watch(
                converterProvider.select((s) => s.fromUnit),
              );

              final category = ref.watch(
                converterProvider.select((s) => s.category),
              );
          ...
          Consumer(
            builder: (context, ref, child) {
              final result = ref.watch(
                converterProvider.select((s) => s.result),
              );

              final toUnit = ref.watch(
                converterProvider.select((s) => s.toUnit),
              );
          ...
          Consumer(
            builder: (context, ref, child) {
              final inputValue = ref.watch(
                converterProvider.select((s) => s.inputValue),
              );
              ...
              return ScaleOnTap(
                onTap: () {
                  Clipboard.setData(
                    ClipboardData(
                      text:
                          '$inputValue ${fromUnit.toUpperCase()} = ${result.toStringAsFixed(5)} ${toUnit.toUpperCase()}',
                    ),
                  );
  ...
  Widget _buildDetailsAndGuides(ConverterCategory category, Color color) {
    ...
    switch (category) {
      ...
      case ConverterCategory.temperature:
        ...
        desc =
            '温度是度量物体冷热程度的物理量，微观上是分子热运动剧烈程度的表现。本站完美对齐了摄氏度 (°C)、华氏度 (°F) 及热力学开尔文 (K) 转换方程式。';
      ...
      case ConverterCategory.sandbox:
        ...
        desc =
            '您可以在此自主设计多重物理参数及兑换因子！输入自定义兑换率系数 (Multiplier) 和数学常数偏移量 (Offset)，本站将实时为您的自定义项目生成整套高精度沙盒对照。';
